    ...
    if ($stmt_dht11->execute()) {
        ...
        if ($stmt_students->num_rows > 0) {
            ...
            if ($stmt_check_log->num_rows > 0) {
                ...
                $deadline_time->modify('+1 day')->setTime(01, 37); // 20:00 is 8:00 PM

                $stmt_logs = $conn->prepare("INSERT INTO logs (uid, name, exit_time, deadline_time) VALUES (?, ?, CURRENT_TIMESTAMP, ?)");
                $formatted_deadline_time = $deadline_time->format('Y-m-d H:i:s');
                $stmt_logs->bind_param("sss", $uid, $name, $formatted_deadline_time);
                ...
                if ($stmt_logs->execute()) {
                    ...
                    try {
                        ...
                        $mail->Password = 'ezql ojsy phme amdw'; // SMTP password or App Password
